...
import { useLiveTimer, activeFieldStyle } from "../context/LiveTimerContext"
...
export default function StructuredTaskPrototype({
  ...
  const { liveTimerTask, getLiveData } = useLiveTimer()
  ...
  function getPartQueueEntries(part) {
    // No longer needed — using getLiveData from context directly
    return []
  }
      ...
      part === "goal"
        ...
        : part === "proof"
          ...
          : part === "priority"
            ? priorityQueueStepId
            : null
    // For "steps", we use the first active step id in the queue
    const liveStepId =
      part === "steps"
        ? (queueStepIds.find((id) => getLiveData(id, queueTaskId)?.isActive) ??
          queueStepIds[0] ??
          null)
        : stepId
    const live = liveStepId ? getLiveData(liveStepId, queueTaskId) : null
    ...
    return (
      <div className="prototype-part-progress" aria-hidden="true">
        <div className="prototype-part-progress__bar">
          <div
            className="prototype-part-progress__fill prototype-part-progress__fill--head"
  ...
  const goalLive = getLiveData(goalQueueStepId, queueTaskId)
  const proofLive = getLiveData(proofQueueStepId, queueTaskId)
  const priorityLive = getLiveData(priorityQueueStepId, queueTaskId)
  // Steps: use the live data of whichever step is currently running
  const activeStepId =
    queueStepIds.find((id) => getLiveData(id, queueTaskId)?.isActive) ??
    queueStepIds[0] ??
    null
  const stepsLive = activeStepId ? getLiveData(activeStepId, queueTaskId) : null
